...
- `fetchStoreProduct()` is shared by version lists, version details and downloads. On failure type 5002 or a successful empty `songList` without a customer error, it falls back to redownload. Unpinned requests first resolve the current iOS external version ID in the account storefront (MDM, then website only for a successful but incomplete catalog result). Explicit historical version IDs are preserved. Only an empty HTTP 500 from pinned redownload tries `updateProduct` once; do not loop back to volumeStore. Validate fallback app, bundle and version identity. Explicit authentication/license errors remain errors.
- Public current-version lookups use `uclient-api.itunes.apple.com` and `apps.apple.com` through Wisp, without Apple account headers or cookies. See `docs/store-fallback.md` for source evidence and verification limits.
...
- Version labels expire 30 days after fetching, without renewal on cache hits, and are capped at 1,000 entries. Clean expired storage at startup, on visibility return, hourly while open, and on cache access. Check in-memory labels before reuse too. Cache cleanup must only target the version metadata keys, never account databases or unrelated settings.
- Automatic version-number lookup is persisted in `asspp-settings` as `autoFetchVersionNumbers` and defaults to false, including older settings. Manual mode shows all version IDs without pagination and only queries explicitly selected rows. Automatic mode uses pages of 20 with a 350 ms paging delay. Both modes reuse valid cached labels and serialize requests; changing modes or leaving the page cancels pending work.
