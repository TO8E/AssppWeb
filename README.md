...
Version History loads the version ID list automatically. **Fetch version numbers automatically** in Settings is off by default, including when upgrading older settings: manual mode shows the entire list without pagination and fetches a version number only when its button is clicked. Cached numbers appear immediately in both modes. The preference persists in the current browser.

With automatic lookup enabled, version numbers resolve sequentially for the current page (20 entries), after paging has stopped for 350 ms. Passing through an intermediate page cancels its pending lookups. Turning automatic lookup off stops queued automatic requests. Failed lookups can be retried individually.

Successful labels are cached in the browser by app and storefront, with up to 1,000 entries and no account credentials. Entries expire 30 days after fetching; reading them does not extend their lifetime. Expired entries are removed on app startup, when a tab becomes visible, hourly while open, and when accessing the cache. If the browser is closed, cleanup resumes on the next visit. Old cache entries without timestamps are discarded on upgrade. The store metadata's `releaseDate` is the app's original release date; it is not displayed as a historical version's publication date.
